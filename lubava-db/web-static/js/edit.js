...
function check_edit_fields(form) {
    var h, str;
    if (!form.group.value && !form.group_force.value) {
        h = form.group;
        str = "Укажите, пожалуйста, категорию.";
    } else if (!form.year.value) {
        h = form.year;
        str = "Укажите, пожалуйста, год написания.";
...
function on_edit_page() {
    ...
    document.getElementById('edit_form').onsubmit = function() {
        return check_edit_fields(this) && set_post_key(this);
    }
}

function on_delete_page() {
    document.getElementById('confirm_delete_form').onsubmit = function() {
        return set_post_key(this);
    }
}
